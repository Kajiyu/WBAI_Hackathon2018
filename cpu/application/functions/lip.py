...
class LIP(object):
    ...
    def __init__(self, use_saliency_flow=False, use_flow_saliency=True):
        ...
        self.last_optical_flow_saliency = None

        self.use_saliency_flow = use_saliency_flow
        self.use_flow_saliency = use_flow_saliency

    def __call__(self, inputs):
        if 'from_retina' not in inputs:
            raise Exception('LIP did not recieve from Retina')
        if 'from_cb' not in inputs:
            raise Exception('LIP did not recieve from CB')
    
        cb_action = inputs['from_cb']
        retina_image = inputs['from_retina'] # (128, 128, 3)
        saliency_map = self._get_saliency_map(retina_image) # (128, 128, 1)
        saliency_gradient_map = self._get_saliency_gradient_map(saliency_map) # (128, 128, 2)

        if not self.use_saliency_flow:
        ...
        if self.use_flow_saliency and optical_flow is not None:
            flow_salicency_map = self._get_saliency_map(optical_flow) # (128, 128, 1)
        else:
            flow_salicency_map = None
        self.last_optical_flow_saliency = flow_salicency_map
        ...
        if optical_flow is None:
            optical_flow = np.zeros((128,128,2))
        if flow_salicency_map is None:
            flow_salicency_map = np.zeros((128,128,1))

        return dict(to_fef=(saliency_map, optical_flow, flow_salicency_map, saliency_gradient_map))
    ...
    def _get_saliency_map(self, image):
        ...
        saliency = cv2.resize(saliency, image.shape[1::-1]).reshape(128,128,1)
        return saliency

    def _get_saliency_gradient_map(self, saliency_map):
        # Configuration of  resolution to differential
        delta = .5
        saliency_map = np.reshape(saliency_map, (128,128))
        # Calc gradient of saliency_map
        (sal_dx,sal_dy)  = np.gradient(saliency_map, delta, delta)

        # Reshape to (128, 128, 2(x,y))
        sal_grad_list = np.vstack((sal_dx.flatten(),sal_dy.flatten()))
        sal_grad_matrix = [list(x) for x in list(zip(*sal_grad_list))]
        sal_grad_map = np.reshape(sal_grad_matrix, (128,128,2))
        return sal_grad_map
